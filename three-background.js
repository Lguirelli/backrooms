...
const SETTINGS = {
  ...
  textures: {
    carpet: { url: './assets/carpet.jpg', repeat: [9, 9] },
    wallDots: { url: './assets/wall.png', repeat: [5, 5] },
    wallChevron: { url: './assets/wall2.png', repeat: [7, 7] },
    ceiling: { url: './assets/ceiling-texture.png', repeat: [6, 6] },
    metalGrid: { url: './assets/wall.png', repeat: [3, 3] }
  },
...
// Câmera atual exportada do projeto.
// O scroll percorre os 300 frames mantendo Y e Z fixos.
...
scene.background = new THREE.Color(SETTINGS.atmosphere.background);
scene.fog = new THREE.FogExp2(SETTINGS.atmosphere.fogColor, SETTINGS.atmosphere.fogDensity);
...
function getPixelRatio() {
  const maxRatio = window.innerWidth < SETTINGS.renderer.mobileBreakpoint
    ? SETTINGS.renderer.pixelRatioMobile
    : SETTINGS.renderer.pixelRatioDesktop;
...
const textures = Object.fromEntries(
  Object.entries(SETTINGS.textures).map(([key, config]) => [
    key,
    loadTexture(config.url, config.repeat[0], config.repeat[1])
  ])
);
...
function getObjectMaterialName(object) {
  if (Array.isArray(object.material)) {
    return object.material.map((mat) => mat?.name || '').join(' ');
  }

  return object.material?.name || '';
}

function chooseMaterialPreset(object) {
  const name = normalize(`${object.name || ''} ${getObjectMaterialName(object)}`);

  if (name.includes('luz') || name.includes('light') || name.includes('lamp') || name.includes('emissive')) return materialPresets.luz;
  if (name.includes('grid') || name.includes('grade') || name.includes('metal') || name.includes('frame')) return materialPresets.grid;
  if (name.includes('chao') || name.includes('piso') || name.includes('floor') || name.includes('carpet')) return materialPresets.chao;
  if (name.includes('teto') || name.includes('ceiling')) return materialPresets.teto;
  if (name.includes('wall_001') || name.includes('wall') || name.includes('parede')) return materialPresets.wall;

  return materialPresets.default;
}

function createLights() {
  const ambient = new THREE.HemisphereLight(
    SETTINGS.lights.ambientSky,
    SETTINGS.lights.ambientGround,
    SETTINGS.lights.ambientIntensity
  );
  scene.add(ambient);

  const keyLight = new THREE.DirectionalLight(SETTINGS.lights.keyColor, SETTINGS.lights.keyIntensity);
  keyLight.position.set(...SETTINGS.lights.keyPosition);
  scene.add(keyLight);

  const fillLight = new THREE.DirectionalLight(SETTINGS.lights.fillColor, SETTINGS.lights.fillIntensity);
  fillLight.position.set(...SETTINGS.lights.fillPosition);
  scene.add(fillLight);

  const backLight = new THREE.DirectionalLight(SETTINGS.lights.backColor, SETTINGS.lights.backIntensity);
  backLight.position.set(...SETTINGS.lights.backPosition);
  scene.add(backLight);

  const fluorescent = new THREE.PointLight(
    SETTINGS.lights.fluorescentColor,
    SETTINGS.lights.fluorescentBaseIntensity,
    SETTINGS.lights.fluorescentDistance
  );
  fluorescent.position.set(...SETTINGS.lights.fluorescentPosition);
...
function addRealLightNearMesh(object) {
  if (meshLightCount >= SETTINGS.lights.maxMeshLights) return;
  ...
  const light = new THREE.PointLight(
    SETTINGS.lights.meshLightColor,
    SETTINGS.lights.meshLightIntensity,
    SETTINGS.lights.meshLightDistance
  );

  light.name = name;
  light.position.copy(center);
  light.position.y += SETTINGS.lights.meshLightYOffset;
...
function getFakeAOColor(baseColor, ao) {
  const darken = 1 - Math.min(
    ao * SETTINGS.fakeAO.strength,
    SETTINGS.fakeAO.maxDarken
  );
...
function applyFakeAmbientOcclusionToMesh(object) {
  if (!SETTINGS.fakeAO.enabled) return;

  const geometry = object.geometry;
  if (!geometry?.attributes?.position || !object.material) return;
  ...
  const floorDistance = Math.max(height * SETTINGS.fakeAO.floorDistanceRatio, SETTINGS.fakeAO.minFloorDistance);
  const ceilingDistance = Math.max(height * SETTINGS.fakeAO.ceilingDistanceRatio, SETTINGS.fakeAO.minCeilingDistance);
  const sideDistanceX = Math.max(width * SETTINGS.fakeAO.sideDistanceRatio, SETTINGS.fakeAO.minSideDistance);
  const sideDistanceZ = Math.max(depth * SETTINGS.fakeAO.sideDistanceRatio, SETTINGS.fakeAO.minSideDistance);
  ...
  for (let i = 0; i < position.count; i += 1) {
    ...
    const floorAO = inverseSmoothstep(0, floorDistance, distanceFromFloor);
    const ceilingAO = inverseSmoothstep(0, ceilingDistance, distanceFromCeiling);

    const sideXAO = Math.max(
      inverseSmoothstep(0, sideDistanceX, distanceFromLeft),
      inverseSmoothstep(0, sideDistanceX, distanceFromRight)
    );

    const sideZAO = Math.max(
      inverseSmoothstep(0, sideDistanceZ, distanceFromBack),
      inverseSmoothstep(0, sideDistanceZ, distanceFromFront)
    );
    ...
    const ao =
      floorAO * SETTINGS.fakeAO.floorWeight * verticalSurface +
      ceilingAO * SETTINGS.fakeAO.ceilingWeight * verticalSurface +
      sideAO * SETTINGS.fakeAO.sideWeight +
      cornerAO * SETTINGS.fakeAO.cornerWeight +
      cavityAO * SETTINGS.fakeAO.cavityWeight * horizontalSurface;

    const finalColor = getFakeAOColor(baseColor, clamp01(ao));
...
function prepareMesh(object) {
  ...
  const selectedMaterial = chooseMaterialPreset(object);
...
function setCameraFromScroll(force = false) {
  const progress = getScrollProgress();

  if (!force && !cameraDirty && Math.abs(progress - lastScrollProgress) < 0.00001) return;
...
function loadModel() {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      setCameraFromScroll(true);
      setStatus('GLTF carregado / visual cinematográfico aplicado');
    },
    (event) => {
      if (!statusEl) return;
...
function updateCinematicFlicker() {
...
function animate() {
  ...
  setCameraFromScroll(false);
  updateCinematicFlicker();
