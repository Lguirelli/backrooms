...
// Movimento original corrigido para o sistema de coordenadas do Three.js.
// Formato: [position.x, pitch, yaw]
...
function setCameraFromScroll() {
  ...
  // Importante:
  // a rotação exportada do Blender tinha o X em aproximadamente 90º,
  // que no Three.js fazia a câmera olhar para o teto.
  // O patch usa pitch corrigido e yaw no eixo Y.
...
loader.load(
  ...
  (gltf) => {
    ...
    setCameraFromScroll();
    setStatus('câmera corrigida: percurso conectado ao scroll');
  },
  (event) => {
    if (!statusEl) return;
    if (event.total) {
