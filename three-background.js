...
const textures = {
  // A textura do carpete é configurada por malha em setupCarpetTextureForMesh(),
  // para manter escala realista mesmo quando o piso tem tamanhos diferentes.
  carpet: loadTexture('./assets/carpet.jpg', {
    repeatX: 1,
    repeatY: 1,
    mirrored: true
  }),
  wallDots: loadTexture('./assets/wall.png', {
    repeatX: 5,
    repeatY: 5
  }),
  wallChevron: loadTexture('./assets/wall2.png', {
    repeatX: 7,
    repeatY: 7
  }),
  ceiling: loadTexture('./assets/ceiling-texture.png', {
    repeatX: 6,
    repeatY: 6
  }),
  metalGrid: loadTexture('./assets/wall.png', {
    repeatX: 3,
    repeatY: 3
  })
};
...
function setupCarpetTextureForMesh(object, material) {
  if (!object.geometry || !material.map) return;
  ...
  const worldWidth = Math.max(size.x * Math.abs(worldScale.x), 0.0001);
  const worldDepth = Math.max(size.z * Math.abs(worldScale.z), 0.0001);

  // Escala aproximada do módulo do carpete no mundo 3D.
  // Menor = mais repetição / fibra mais miúda.
  // Maior = menos repetição / textura mais ampla.
  const carpetTileSize = 1.15;

  const repeatX = Math.max(1, worldWidth / carpetTileSize);
  const repeatY = Math.max(1, worldDepth / carpetTileSize);

  const carpetMap = material.map.clone();
  carpetMap.wrapS = THREE.MirroredRepeatWrapping;
  carpetMap.wrapT = THREE.MirroredRepeatWrapping;
  carpetMap.repeat.set(repeatX, repeatY);
  carpetMap.anisotropy = maxAnisotropy;
  carpetMap.colorSpace = THREE.SRGBColorSpace;
  carpetMap.flipY = false;
  carpetMap.needsUpdate = true;

  material.map = carpetMap;
  material.color.set(0xf2efe6);
...
function applyMaterials(root) {
  root.traverse((object) => {
    ...
    if (selectedMaterial.name === 'web_floor_carpet') {
      setupCarpetTextureForMesh(object, object.material);
    }
